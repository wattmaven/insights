...
daily_completeness_mask = gaps.complete(
    data["value_normalized"], minimum_completeness=min_completeness
)
...
completeness_trim_mask = gaps.trim_incomplete(
    data["value_normalized"], days=number_consecutive_days
)
# Re-visualize the time series with the data masked by the trim mask
data[completeness_trim_mask]["value_normalized"].plot()
data[~completeness_trim_mask]["value_normalized"].plot()
plt.legend(labels=[True, False], title="Daily Data Passing")
plt.xlabel("Date")
